...
class SpanFPreRecMetric(MetricBase):
    ...
    def __init__(self, tag_vocab, pred=None, target=None, seq_len=None, encoding_type=None, ignore_labels=None,
                 only_gross=True, f_type='micro', beta=1,nam_nom=False):
        r"""
        ...
        self._spanF_true_positives = 0
        self._spanF_false_positives = 0
        self._spanF_false_negatives = 0

        self.nam_nom = nam_nom  # if weibo, set it 'True'
        if self.nam_nom:
            self.only_gross = False
    ...
    def evaluate(self, pred, target, seq_len):
        ...
        for i in range(batch_size):
            ...
            SpanF_pred_spans = [span[1] for span in pred_spans]
            SpanF_gold_spans = [span[1] for span in gold_spans]
            ...
            # compute spanF
            for span in SpanF_pred_spans:
                if span in SpanF_gold_spans:
                    self._spanF_true_positives += 1
                    SpanF_gold_spans.remove(span)
                else:
                    self._spanF_false_positives += 1
            for span in SpanF_gold_spans:
                self._spanF_false_negatives += 1
    ...
    def get_metric(self, reset=True):
        ...
        if self.f_type == 'micro':
            ...
            spanF = self._compute_f_pre_rec(self._spanF_true_positives,
                                            self._spanF_false_negatives,
                                            self._spanF_false_positives)
            type_acc = sum(self._true_positives.values()) / self._spanF_true_positives

            evaluate_result['f'] = f
            evaluate_result['pre'] = pre
            evaluate_result['rec'] = rec
            evaluate_result['spanF'] = spanF
            evaluate_result['type_acc'] = type_acc

        if self.f_type == 'micro' and self.nam_nom == True:
            nam_tp, nam_fn, nam_fp = 0, 0, 0
            nom_tp, nom_fn, nom_fp = 0, 0, 0
            for tag in tags:
                if tag.lower().endswith('nam'):
                    nam_tp += self._true_positives[tag]
                    nam_fn += self._false_negatives[tag]
                    nam_fp += self._false_positives[tag]
                elif tag.lower().endswith('nom'):
                    nom_tp += self._true_positives[tag]
                    nom_fn += self._false_negatives[tag]
                    nom_fp += self._false_positives[tag]
            evaluate_result['nam_f'] = self._compute_f_pre_rec(nam_tp, nam_fn, nam_fp)
            evaluate_result['nom_f'] = self._compute_f_pre_rec(nom_tp, nom_fn, nom_fp)
