...
class Transformer_Encoder_Layer(nn.Module):
    ...
    def forward(self, inp, seq_len, lex_num=0,pos_s=None,pos_e=None,rel_pos_embedding=None):
        ...
        output = self.layer_postprocess(output,inp)
        output1 = self.layer_preprocess(output)
        output = self.ff(output1)
        output = self.layer_postprocess(output,output1)
...
class Layer_Process(nn.Module):
    ...
    def forward(self, output,inp=None):
        # output = inp
