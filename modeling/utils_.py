...
class TrieNode:
    def __init__(self):
        self.children = collections.defaultdict(TrieNode) #defaut: returns a TrieNode if not exists key; or exists, returns
        self.is_w = False
...
class Trie:
    ...
    def get_lexicon(self,sentence):
        result = []
        for i in range(len(sentence)): # i '科'
            current = self.root
            for j in range(i, len(sentence)): # j '全'
                current = current.children.get(sentence[j]) # 方   、长、研、...
                if current is None:
                    break

                if current.is_w: # 科技是个单词吗？
