...
def flat_main(batch=10,lr=1e-3,head_dim=20,head=8,warmup=0.1,dataset='weibo',device='0',ck=None,ctr=False,output_dir=None,weight_decay=0,temp=0.07,only_head=0,layer=1,epoch=100,fix_bert_epoch=20,bert_base='cn-wwm',seed=1080956,max_patience=15,optimizer='sgd'):
    ...
    if args.status == 'train':
        ...
        # test_metrics = trainer.tester_test.test_spanF()['SpanFPreRecMetric']
        #
        # for k,v in test_metrics.items():
        #     writer.write(k+': '+str(v)+'\n')
        #
        # # test accordding to length
        # Len_test = True
        # if Len_test:
        #     trainer = Trainer(datasets['train'], model, optimizer, loss, args.batch,
        #                       n_epochs=args.epoch,
        #                       dev_data=datasets['dev'], test_data=datasets['test'],
        #                       metrics=metrics,
        #                       device=device, callbacks=callbacks, dev_batch_size=1,
        #                       test_use_tqdm=False, check_code_level=-1,
        #                       update_every=args.update_every, is_ctr=is_ctr, save_path=output_dir, train_writer=writer)
        #     maxLen_list=[40,60,80,100,128] #128: only means len>100, without any relation of limit 128
        #     # trainer.tester_test.test_len_cont()
        #     len_to_score={}
        #     for maxLen in maxLen_list:
        #         score = trainer.tester_test.test_len(maxLen)['SpanFPreRecMetric']
        #         len_to_score[maxLen]=score
        #         print(score)
        #     # print(len_to_score)
        #     for k, v in len_to_score.items():
        #         writer.write(str(k)+ ': ' + str(v) + '\n')
        #     # writer.write(str(len_to_score)+'\n')
        #
        #
        # # if is_ctr:
        # #     best_f1=best_score['loss']
        # # else:
        # writer.flush()
        ...
        case_study=True
        if case_study:
            output_path=os.path.join(output_dir,args.dataset+'_'+str(best_f1))
            f=open(output_path, 'w+')
            f_all=open(output_path+'_all', 'w+')


            writer_embed=open(output_path+'_emb', 'w+')
            trainer.tester_test.record(vocabs,f,f_all,writer_embed=writer_embed)
